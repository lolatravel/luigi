...
class SimpleWorker(Worker):
    ...
    def __init__(self, worker_id, last_active=None):
        super(SimpleWorker, self).__init__(worker_id)
...
class TaskState(object):
    ...
    def set_status(self, task, new_status, config=None):
        ...
        if new_status != task.status:
            self.change_status(task, new_status)
    ...
    def _remove_workers_from_tasks(self, worker_ids, remove_stakeholders=True):
        for task in self.get_active_tasks():
            if remove_stakeholders:
                task.stakeholders.difference_update(worker_ids)
            task.workers -= worker_ids
            self.save_task(task)
...
class SimpleTaskState(TaskState):
    ...
    def get_worker(self, worker_id):
        return self._active_workers.setdefault(worker_id, SimpleWorker(worker_id))
...
class Scheduler(object):
    ...
    def _prune_workers(self):
        ...
        for worker in self._state.get_active_workers():
            if worker.prune(self._config):
                ...
                remove_worker_ids.append(worker.id)

        self._state.inactivate_workers(remove_worker_ids)
    ...
    def add_task(self, task_id=None, status=PENDING, runnable=True,
        ...
        * update priority when needed
        ...
        if runnable and status != FAILED and worker.enabled:
            ...
            self._state.get_worker(worker_id).add_task(task)
    ...
    def get_work(self, host=None, assistant=False, current_tasks=None, worker=None, **kwargs):
        ...
        if len(batched_tasks) > 1:
            ...
            self._state.save_task(best_task)
        ...
        if self._state.has_task(task_id):
            ...
            if task.status == RUNNING and task.batch_id is not None:
                for batch_task in self._state.get_batch_running_tasks(task.batch_id):
                    ...
                    self._state.save_task(batch_task)
